...
func main() {
	...
	for _, veh := range inventory {
		...
		case truck:
			v.truckDetails()
		default:
			fmt.Printf("Are you sure this Vehicle Tpe exists")
		}
	}
}
	...
	for _, v := range f.Models {
		...
		for _, msg := range v.Feedback {
			if text := strings.Split(msg, " "); len(text) >= 5 {
				...
				case vehRating >= 4.0 && vehRating <= 8.00:
...
func showRating(model string) {
	...
	for m, r := range vehicleResult {
		if m == model {
			fmt.Printf("Total Ratings:%v\tPositive:%v\tNegaive:%v\tNeutral:%v", r.feedbackTotal, r.feedbackPositive, r.feedbackNegative, r.feedbackNeutral)
			ratingFound = true
		}
	}
	if !ratingFound {
